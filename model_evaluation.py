...
from sklearn.metrics import confusion_matrix, roc_curve, auc, plot_confusion_matrix, plot_precision_recall_curve, classification_report
...
def k_fold(X_train, y_train, colors, seed, PATH):
    ...
    ## the  above link is a good source fo neat way to loop through multiple classifiers, and plot confusion matric etc.
    ## the 2nd link shows how to use  subplots
...
#         icgc_df_scaled = scaler.transform(icgc_df)
...
#             train_score.append(classifier.score(xtrain_scaled, ytrain))
#             test_score.append(classifier.score(xtest_scaled, ytest))
            print(classifier.score(xtest_scaled, ytest))
#             icgc_score.append(classifier.score(icgc_df_scaled, icgc_labels))
#             print(classifier.score(icgc_df_scaled, icgc_labels))
...
#             icgc_acc_dict[f'{type(classifier).__name__}'] += classifier.score(icgc_df_scaled, icgc_labels)
...
#                 print(confusion_matrix(ytest, classifier.predict(xtest_scaled)))
            else:
...
#         print(test_score)
    ...
    temp_pd = pd.DataFrame({'Avg Train Accuracy':list(train_acc_dict.values()),'AUC':list(roc_score_dict.values()), 'Avg Test Accuracy':list(test_acc_dict.values())}, index=classifier_name)
    temp_pd = temp_pd/k
    display(temp_pd)
    ruler = HTML("<hr><hr>")
    display(ruler)
...
#             gene_val = group['normalized_read_count'].iloc[gene_loc]
            gene_val = group['raw_read_count'].iloc[gene_loc]
            _.append(gene_val)
        _.append(1)
        _.append(name)
...
#             gene_val = group['normalized_read_count'].iloc[gene_loc]
    ...
    display(HTML("After log2 normalise....."))
...
#------------------
# best accuracy so far in mlp
#Avg Train Accuracy	Avg Test Accuracy	Avg ROC Score
#MLPClassifier	1.000000	0.955713	0.988223
#LogisticRegression	0.967424	0.949502	0.986397
#RandomForestClassifier	1.000000	0.945062	0.985018
#Support Vector Classifier	0.967523	0.952157	0.986683
    #sklearn_classifiers = [

        #MLPClassifier(hidden_layer_sizes=(1024, 512, 256, 128), random_state=seed, batch_size=batch_size_df, verbose=False, learning_rate_init=learning_rate, max_iter=epochs),  ## predict_proba
        #LogisticRegression(random_state=seed, max_iter=epochs, solver='saga', penalty='elasticnet', l1_ratio=0.4),  ## predict_proba
        #RandomForestClassifier(max_depth=10, random_state=seed), ## predict_proba
...
# def nn_classifier(xtrain, ytrain, xtest, ytest):
